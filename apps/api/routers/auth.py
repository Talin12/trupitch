...
async def organizer_register(
    ...
    existing = (
        await db.execute(select(Organizer).where(Organizer.email == payload.email))
    ).scalar_one_or_none()
    if existing is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="An organizer with this email already exists",
        )
    organizer = Organizer(
        email=payload.email,
        name=payload.name,
    ...
    return TokenResponse(
        access_token=create_organizer_token(organizer.id), organizer=organizer
    )


@router.post("/organizer/login", response_model=TokenResponse)
async def organizer_login(
    payload: OrganizerLogin, db: AsyncSession = Depends(get_db)
) -> TokenResponse:
    organizer = (
        await db.execute(select(Organizer).where(Organizer.email == payload.email))
    ).scalar_one_or_none()
    if organizer is None or not verify_password(
        payload.password, organizer.password_hash
    ):
        raise HTTPException(
